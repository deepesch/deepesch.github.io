-- Calculate the total number of edits and net size change per user, per hour
...
temp = FOREACH unique GENERATE CONCAT((chararray)ToString(ts, 'yyyy-MM-dd\'T\'HH'), ':00:00Z') AS ts, user, editsize;

B = FOREACH temp GENERATE FLATTEN($0) AS eventtime, user, editsize;

C = group B by (eventtime, user);

-- Bin each time interval (hour), for each user, with number of events and net editsize
